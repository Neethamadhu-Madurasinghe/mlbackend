...
dotted_shape_model_bp = Blueprint('dotted_shape_model', __name__)
...
# Load the model
model_path = os.path.join(os.path.dirname(__file__), 'dotted_shapes.h5')
...
@dotted_shape_model_bp.route('/predict', methods=['POST'])
def predict():
    try:
        print("Dotted Shapes")
